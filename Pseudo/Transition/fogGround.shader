...
uniform float cloudDensity = 1.0; 	// overall density [0,1]
uniform float noisiness = 0.35; 	// overall strength of the noise effect [0,1]
uniform float speed = 0.1;			// controls the animation speed [0, 0.1 ish)
uniform float cloudHeight = 2.5; 	// (inverse) height of the input gradient [0,...)
...
void fragment()
{
	...
	COLOR = vec4(q,q,q,smoothstep(.1,.9,q));
}
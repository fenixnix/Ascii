...
const vec2 MOD2 = vec2(0.3247122237916646, 0.134707348285849);

//const vec3 sunLight = normalize( vec3(  0.35, 0.2,  0.3 ) );
uniform vec3 sunLight = vec3(0.35, 0.2, 0.3);
uniform vec2 iResolution = vec2(2048.,2048.);
const vec3 sunColour = vec3(1.0, .8, .25);// let's our sun will be more yellow
...
//--------------------------------------------------------------------------
vec3 GrassBlades(in vec3 rO, in vec3 rD, in vec3 mat, in float dist, in float iTime)
{
	float d = 0.0;
...
//--------------------------------------------------------------------------
vec3 TerrainColour(vec3 pos, vec3 dir,  vec3 normal, float dis, float type, float iTime)
{
	vec3 mat;
	if (type == 0.0)
	{
		...
		mat = GrassBlades(pos, dir, mat, dis, iTime) * t;
...
vec3 drawImage(in vec3 cameraPos, in vec3 dir, float iTime)
{
	float distance;
	float type;
	...
	else
	{
		...
		return TerrainColour(pos, dir, nor, distance, type, iTime);
	}
}
...
void fragment()
{
	float gTime = (TIME*5.0+2352.0)*.006;
        vec2 xy = UV;// FRAGCOORD.xy / iResolution.xy;
	...
	vec3 col = drawImage(cameraPos, dir, TIME);

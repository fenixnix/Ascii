...
void fragment() {
	...
	vec4 iResolution = vec4(resolution);
	float iTime = TIME;
    ...
    color += (texture(tx,uv*0.6+vec2(.5,.1)).xyz*0.01*pow((r.y+r.y)*.65,5.0)+0.055)*mix( vec3(.9,.4,.3),vec3(.7,.5,.2), uv.y);

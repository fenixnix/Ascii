...
vec3 raymarch_lsdlive(vec3 ro, vec3 rd, vec2 uv, float g) {
	vec3 p;
	float t = 0., ri;
...
void glitch(inout vec2 uv, float start_time_stamp, float end_time_stamp)
{
	...
	float res = mix(10., 100.0, random(vec2(float(offset))));
	...
	f seed = mod(time, 2.) > 1. ? seedX : seedY;


	// glitch splitter
	uv.x += (random(vec2(float(seed))) * 2.0 - 1.0)

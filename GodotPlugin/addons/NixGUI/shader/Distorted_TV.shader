...
uniform float rgbOffsetOpt = 0.2;
...
void fragment()
{
    ...
    for (float _y = -1.0; _y <= 1.0; _y += 1.0) {
        ...
        float dist = _y/200.0;
    	staticVal += staticV(vec2(uv.x,uv.y+dist),iTime)*(maxDist-abs(dist))*1.5;
    }
        
	staticVal *= bottomStaticOpt;

	float red = texture(SCREEN_TEXTURE,vec2(uv.x + xOffset -0.01*rgbOffsetOpt,y)).r;
	float green = texture(SCREEN_TEXTURE, vec2(uv.x + xOffset, y)).g;
	float blue = texture(SCREEN_TEXTURE, vec2(uv.x + xOffset +0.01*rgbOffsetOpt,y)).b;
	vec3 color = vec3(red,green,blue) + staticVal;

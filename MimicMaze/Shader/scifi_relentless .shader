...
float circles(vec2 p, float time)
{
	float v, w, l, c;
	vec2 pp;
	l = length(p);
...
void fragment()
{
	...
	inten = pow(1.0 - abs(sd), 20.0) + pow(sky(rd,time), 5.0) * step(0.0, rd.y) * 0.2;
	...
	for(int j = 0; j < 20; j ++)
	{
		...
		vec3 its1 = intersect(ro, rd, bp, vec3(1.0, 0.0, 0.0), vec3(0.0, 0.0, 1.0));
		...
		if(its1.x > 0.0)
		{
			...
			float foc = 0.0;
			float v1 = 1.0 - smoothstep(0.0, 0.03, abs(d) - 0.001);
			float g1 = min(exp(d * -20.0), 2.0);

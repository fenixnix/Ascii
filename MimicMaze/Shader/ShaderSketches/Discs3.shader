...
vec3 saturate3(vec3 x) { return clamp(x, 0, 1); }
...
vec3 palette(float z, float time)
{
    vec3 a = vec3(0.90, 0.60, 0.69);
    vec3 b = vec3(0.17, 0.41, 0.41);
    return saturate3(a + b * sin(z * 9. + time * 2.));
}
...
void fragment()
{
    ...
    vec2 p = (FRAGCOORD.xy - resolution / 2.) / scale;
    vec2 p1 = fract(p) - 0.5;
    ...
    vec3 c1 = palette(z1,time);
    vec3 c2 = palette(z2,time);

...
float SnowLayer(vec2 uv,float iTime){
	vec2 GA = vec2(-1.8,.9)*iTime*speed;
...
void fragment()
{
	...
	float Snowout = SnowLayer(uv*.9-.5,iTime);//modify
	
	//background layer
	//Snowout += 0.35+(slope*(suns+0.3))+(sunsh*0.6);
	
	COLOR = vec4(Snowout*0.9, Snowout, Snowout*1.1, step(.3,Snowout));
}
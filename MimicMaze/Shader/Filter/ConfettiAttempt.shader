...
uniform int N_POPS = 5;
uniform float TIME_BETWEEN_POPS = 1.2;
uniform float TIME_BETWEEN_POPS_RANDOM = 0.6;
uniform int N_CONFETTI = 16;
const float PI = 3.1415926535;
uniform float SQUARE_HARDNESS = 10.0;
uniform float GLOW_INTENSITY = 0.8;
uniform float HUE_VARIANCE = 0.2;

uniform float V_INITIAL = 600.0;
uniform float V_RANDOM = 300.0;
uniform float GRAVITY = 17.81;
uniform float TERMINAL_VELOCITY = 60.0;
uniform float TERMINAL_VELOCITY_RANDOM = 10.0;

uniform float GLOW_WHITENESS = 0.56;
uniform float SQUARE_WHITENESS = 0.86;
